...
if ($_FILES["newsphoto"]["size"] < 1000000)
  ...
  if ($_FILES["newsphoto"]["error"] > 0)
    ...
    {
    if (file_exists("../images/banner/" . $_FILES["newsphoto"]["name"]))
      {
      ...
      "../images/banner/" . $_FILES["newsphoto"]["name"]);
      }
    }
  }
else
...
?>
<!--添加上传图片-->
<?php echo "你好，你上传的".$_FILES["newsphoto"]["name"]."已经上传至$sheet"; ?>
...
$sql="UPDATE  `vptdatebase`.`navs` SET  `$sheet` =  '".$_FILES["newsphoto"]["name"]."' WHERE  `navs`.`id` =3;";
